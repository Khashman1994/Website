...
// ── Country / city → ISO 3166-1 alpha-2 map ─────────────────────────────────
// Keep in sync with ARAB_COUNTRIES_CITIES in components/ui/LocationSelector.tsx.
// We can't import from there because this file runs in plain Node (no React).
...
const COUNTRY_ISO_MAP: Record<string, string> = {
  ...
  'kaédi':'mr','kaedi':'mr',
  // Disambiguated Tripoli
  'tripoli, lebanon':'lb','tripoli, libya':'ly',
};

/**
 * Resolve a country / city / "City, Country" string to an ISO 3166-1 alpha-2 code.
 * Returns null when the input is unrecognised so we don't poison the column.
 */
...
function mapJSearch(j: any, sector: string) {
  return {
    ...
    country:         toIso(j.job_country) ?? toIso(j.job_city),
...
function mapSerpApi(j: any, sector: string, location: string) {
  ...
  return {
    ...
    country:         toIso(location) ?? toIso(j.location),
